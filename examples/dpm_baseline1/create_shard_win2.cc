...
void createShard(const char *input, int trainSize, int validSize,
    ...
    int offset = 0;
    offset += generateShardFile(featureMatrix, "dpm_baseline1_train_shard_win2", trainSize,
                                windowNum, featureDim, offset, nricVec, labelVec);
    offset += generateShardFile(featureMatrix, "dpm_baseline1_valid_shard_win2", validSize,
                                windowNum, featureDim, offset, nricVec, labelVec);
    offset += generateShardFile(featureMatrix, "dpm_baseline1_test_shard_win2", testSize,
                                windowNum, featureDim, offset, nricVec, labelVec);
