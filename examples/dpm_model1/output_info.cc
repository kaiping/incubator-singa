...
int printFeatures(const std::string &filePath, float *featureMatrix, float *labelVec,
    ...
    for(int idx1 = 0; idx1 < windowNum; ++idx1) {
        for(int idx2 = 0; idx2 < featureDim; ++idx2) {
            out << "Feature" << idx1 * featureDim + idx2 << ",";
        }
    }
    out << "Label" << std::endl;
    ...
    for (int i = start_idx; i < end_idx; ++i) {
        for (int j = 0; j < windowNum; ++j) {
            for (int k = 0; k < featureDim; ++k) {
                //if ((featureMatrix + i * patientWidth + j * featureDim)[k] > 0)
...
void OutputInfo (const char *input) {  // input is the output text file of python
    ...
    offset += generateShardFile(featureMatrix, "dpm_model1_test_shard", testSize,
                                windowNum, featureDim, offset, nricVec, labelVec);*/
    printFeatures("output_info.txt", featureMatrix, labelVec, 0, patientNum, windowNum, featureDim);
