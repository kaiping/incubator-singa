# This is the code when we add the functionality of randomly picking testing samples
...
if __name__ == '__main__':
    ...
    # select particular subsection as test data, -1 means randomly pick
    if -1 == test_index:
        test_size = int(total_sample * test_ratio)
        test_sample_set = set(random.sample(range(total_sample), test_size))
    else:
    ...
    with open(train_file_path, 'w') as f:
        f.writelines(sample_lines[idx] + '\n' for idx in range(total_sample) if idx not in test_sample_set)