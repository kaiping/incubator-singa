...
/******************** Implementation for DPMCombineSoftmaxLossLayer******************/
    ...
    void DPMCombineSoftmaxLossLayer::ComputeFeature(Phase phase, Metric* perf) {
      ...
      auto data = Tensor2(&data_);
      data = prob_win1 * (1.0 / 9.0) + prob_win2 * (1.0 / 3.0) + prob_win3 * (5.0 / 9.0);
    ...
    void DPMCombineSoftmaxLossLayer::ComputeGradient(Phase phase) {
      ...
      for(int n = 0;n < batchsize_;n++){
        for(int idx = 0;idx < dim_; idx++){
            gsrcptr_1[n * dim_ + idx] = gsrcptr_1[n * dim_ + idx] * softmax1[n*dim_+static_cast<int>(label[n])] / (9.0 * datainfo[n*dim_+static_cast<int>(label[n])]);
            ...
            gsrcptr_3[n * dim_ + idx] = gsrcptr_3[n * dim_ + idx] * softmax3[n*dim_+static_cast<int>(label[n])] * 5.0 / (9.0 * datainfo[n*dim_+static_cast<int>(label[n])]);
        }
      }
